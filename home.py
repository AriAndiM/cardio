...
with st.container():
    ...
    if choose == "Home":
        ...
        st.caption('<p>atau bisa dituliskan :</p>', unsafe_allow_html = True)
        st.latex(r'''posterior = \frac{prior * likelihood}{evidence}''')
        st.caption('<p>P adalah nilai dari </p>', unsafe_allow_html = True)
        ...
        st.caption('<p style = "text-align: justify;">sebagian besar digunakan untuk mengklasifikasi kategori dokumen. Sebuah dokumen dapat dikategorikan bertema olahraga, politik, teknologi, atau lain-lain berdasarkan frekuensi kata-kata yang muncul dalam dokumen.</p>', unsafe_allow_html = True)
        st.markdown('<b>b. Bernouli Naive Bayes</b>', unsafe_allow_html = True)
        st.caption('<p style = "text-align: justify;">Tipe ini mirip dengan tipe Multinomial, namun klasifikasinya lebih berfokus pada hasil ya/tidak. Prediktor yang di-input adalah variabel boolean. Misalnya, prediksi atas sebuah kata muncul dalam teks atau tidak.</p>', unsafe_allow_html = True)
        st.caption('<b>c. Gaussian Naive Bayes</b>', unsafe_allow_html = True)
        st.caption('<p style = "text-align: justify;">Distribusi Gaussian adalah asumsi pendistribusian nilai kontinu yang terkait dengan setiap fitur berisi nilai numerik. Ketika diplot, akan muncul kurva berbentuk lonceng yang simetris tentang rata-rata nilai fitur.</p>', unsafe_allow_html = True)
